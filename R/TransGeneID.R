  ...
  if(fromOrg==toOrg){#### GeneID Transformation within organisms ####
    ...
    if(all(c(fromType, toType) %in% c("entrez", "symbol", "ensembl"))){
      ...
      if(sum(attrs==toType)==0){
        ...
        idx = idx1
        if(sum(idx1)>2) idx = idx1&grepl("_id", attrs)
        toType = ifelse(sum(idx)>0, attrs[idx][1], attrs[idx1][1])
    ...
    if(all(c(fromType, toType) %in% c("symbol", "entrez"))){
      ...
      if(sum(attrs_2==toType)==0){
        ...
        toType = ifelse(sum(idx)>0, attrs_2[idx][1], attrs_2[idx1][1])
        if(toType=="hgnc_symbol" & toOrg=="mmu") toType = "mgi_symbol"
      }
      ## retrieve the data
...
getGeneAnn <- function(org = "hsa", update = FALSE, release = NULL){
  ...
  # if(org=="hsa"){
  #   locfname2 = file.path(.gsbundle_cache(), "HGNC_GeneID_annotation.txt.gz")
  ...
  if (!is.null(release)) {
    ...
    rest_response <- readLines(url("https://rest.ensembl.org/info/data?content-type=application/json"), warn = FALSE)
    rest_json <- paste(rest_response, collapse = "")
    releases <- as.integer(unlist(regmatches(rest_json, gregexpr("[0-9]+", rest_json))))
    version <- max(releases, na.rm = TRUE)
  }
